...
export default function ProductsPage() {
  ...
  const handleOk = async () => {
    try {
      ...
      if (editing) {
        ...
        msg.success(t('common.edit') + ' OK');
      } else {
        await create(values);
        msg.success(t('products.create') + ' OK');
      }
      setModalOpen(false);
    } catch {
      /* validation */
  ...
  const columns: ColumnsType<Product> = [
    {
      title: t('products.imageUrl'),
      dataIndex: 'image_url',
      width: 70,
      render: (v: string | null) =>
        v ? (
          <Image src={v} width={36} height={36} style={{ borderRadius: 6, objectFit: 'cover' }} />
        ) : (
          <div style={{ width: 36, height: 36, borderRadius: 6, background: 'rgba(255,255,255,0.05)' }} />
        ),
    },
      ...
      ? [
          {
            ...
            render: (_: any, r: Product) => (
              <Space>
                ...
                {perm.canDelete && (
                  <Popconfirm title={t('users.confirmDelete')} onConfirm={() => handleDelete(r.id)}>
    ...
    <div className="page-transition">
      ...
      <Card style={{ background: 'transparent' }}>
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (cnt) => `${cnt} ${t('common.yes')}`,
            onChange: (p, ps) => {
              setPage(p);
              setPageSize(ps);
            },
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Space style={{ display: 'flex' }} size={16}>
            <Form.Item name="category" label={t('products.category')} style={{ width: 200 }}>
              <DictSelect dictCode="product_category" />
            </Form.Item>
            <Form.Item name="status" label={t('products.status')} style={{ width: 120 }}>
              <DictSelect dictCode="product_status" />
            </Form.Item>
          </Space>
          <Form.Item name="image_url" label={t('products.imageUrl')}>
