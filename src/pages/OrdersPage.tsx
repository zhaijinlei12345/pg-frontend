...
export default function OrdersPage() {
  ...
  const handleStatus = async (id: number, newStatus: string) => {
    try {
      ...
      msg.success(t('orders.markComplete') + ' OK');
    } catch {
      msg.error('Failed');
    }
  };
    ...
    },
    {
      ...
      render: (_, r: Order) => (
        <Space>
          ...
          {perm.canManage && r.status === 'pending' && (
            <Button type="link" size="small" onClick={() => handleStatus(r.id, 'paid')}>
              {t('orders.markPaid')}
            </Button>
          )}
          {perm.canManage && r.status === 'paid' && (
            <Button type="link" size="small" onClick={() => handleStatus(r.id, 'shipped')}>
              {t('orders.markShipped')}
            </Button>
          )}
          {perm.canManage && r.status === 'shipped' && (
            <Button type="link" size="small" onClick={() => handleStatus(r.id, 'completed')}>
          ...
          {perm.canDelete && (r.status === 'pending' || r.status === 'cancelled') && (
            <Popconfirm
              ...
              onConfirm={async () => {
                try {
                  await remove(r.id);
                  msg.success(t('orders.delete') + ' OK');
                } catch {
                  msg.error('Failed');
                }
              }}
  ...
  return (
    <div className="page-transition">
      ...
      <Drawer
        title={`${t('orders.detail')} - ${detail?.order_no || ''}`}
        open={detailOpen}
      ...
      >
        {detail && (
          <>
            ...
            <Title level={5} style={{ color: '#e1e4ed', marginTop: 20 }}>
              {t('orders.items')}
            </Title>
            <Table
            ...
            <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
              ...
              {detail.status === 'paid' && (
                <Button
                  type="primary"
                  onClick={() => {
                    handleStatus(detail.id, 'shipped');
              ...
              {detail.status === 'shipped' && (
                <Button
                  type="primary"
                  onClick={() => {
                    handleStatus(detail.id, 'completed');
