...
export default function AuditLogsPage() {
  ...
  function actionTag(a: string) { const e = actionDict?.entries.find(x => x.key === a); return <Tag color={e?.color || undefined}>{e?.label || a}</Tag>; }
  ...
  const columns: ColumnsType<AuditLog> = [
    ...
    { title: 'Details', dataIndex: 'details', key: 'details', ellipsis: true, render: (v: string | null) => { if (!v) return '-'; try { return JSON.stringify(JSON.parse(v)); } catch { return v; } } },
    { title: t('users.createdAt'), dataIndex: 'created_at', key: 'created_at', width: 180, render: (v: string) => new Date(v).toLocaleString('zh-CN') },
  ];
