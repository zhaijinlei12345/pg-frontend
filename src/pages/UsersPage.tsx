...
export default function UsersPage() {
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteUser(id);
      msg.success(t('users.delete') + ' OK');
    } catch {
      msg.error(t('users.delete') + ' Failed');
    }
  };
  ...
  const columns: ColumnsType<User> = [
    ...
    { title: t('users.role'), dataIndex: 'role', key: 'role', width: 90, render: roleTag },
    {
      ...
      render: (v: string) => new Date(v).toLocaleString(i18n.language === 'en-US' ? 'en-US' : 'zh-CN'),
    },
    ...(perm.canWrite
      ? [
          {
  ...
  const avgAge =
    users.length > 0
      ? Math.round(
          users.filter((u) => u.age != null).reduce((s, u) => s + (u.age || 0), 0) /
            users.filter((u) => u.age != null).length,
        )
      : 0;
  ...
  return (
    <div className="page-transition">
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic title={t('users.totalUsers')} value={total} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic title={t('users.currentPage')} value={`${page} / ${Math.max(1, Math.ceil(total / pageSize))}`} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic title={t('users.avgAge')} value={avgAge || '-'} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic
              ...
              value={isAuthenticated ? t('users.loggedIn') : t('users.notLoggedIn')}
            />
          </Card>
        </Col>
      </Row>
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item
            name="age"
            label={t('users.age')}
            rules={[{ type: 'number', min: 0, max: 200, transform: (v: string) => (v ? Number(v) : undefined) }]}
          >
            <Input />
          </Form.Item>
          {perm.canDelete && (
