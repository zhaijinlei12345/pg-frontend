    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 120, paddingTop: 8 }}>
      {data.map((d, i) => (
        ...
        >
          ...
          />
          <Text style={{ fontSize: 10, color: '#6b7280', marginTop: 6, whiteSpace: 'nowrap' }}>{d.date.slice(5)}</Text>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="page-transition">
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        ...
        <Col xs={24} md={8}>
          <Card
            title={
              <span>
                <PieChartOutlined /> {t('dashboard.roleDist')}
              </span>
            }
              ...
              return (
                <div key={r.role} style={{ marginBottom: 16 }}>
                  ...
                  <Progress
                    percent={pct}
                    showInfo={false}
                    strokeColor={ROLE_COLORS[r.role] || '#6366f1'}
        ...
        <Col xs={24} md={8}>
          <Card
            title={
              <span>
                <OrderedListOutlined /> {t('dashboard.orderStatus')}
              </span>
            }
            style={{ background: 'transparent' }}
          >
              ...
              return (
                <div key={s.status} style={{ marginBottom: 14 }}>
                  ...
                  <Progress
                    percent={pct}
                    showInfo={false}
                    strokeColor={entry?.color || '#6366f1'}
                    trailColor="rgba(255,255,255,0.04)"
                    size="small"
                  />
                </div>
        ...
        <Col xs={24} md={8}>
          <Card
            title={
              <span>
                <RiseOutlined /> {t('dashboard.trend')}
              </span>
            }
            style={{ background: 'transparent' }}
          >
      ...
      <Card
        title={
          <span>
            <FileTextOutlined /> {t('dashboard.recentOps')}
          </span>
        }
        style={{ background: 'transparent' }}
      >
