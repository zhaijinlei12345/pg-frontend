...
export default function ProfilePage() {
  ...
  const handleProfile = async (values: { name: string; email: string; age?: string }) => {
    ...
    try {
      ...
      msg.error(err.response?.data?.message || 'Failed');
    } finally {
    ...
    <div className="page-transition" style={{ maxWidth: 640 }}>
      ...
      <Card title={t('profile.basicInfo')} style={{ background: 'transparent', marginBottom: 24 }}>
        ...
        >
          ...
          <Descriptions.Item label={t('profile.role')}>
            <Tag color={roleEntry?.color || undefined}>{roleEntry?.label || user?.role || 'User'}</Tag>
          </Descriptions.Item>
        </Descriptions>
        ...
        >
          ...
          <Form.Item name="age" label={t('profile.age')}>
            <Input type="number" />
          </Form.Item>
      ...
      <Card title={t('profile.changePwd')} style={{ background: 'transparent' }}>
        <Form form={pwdForm} layout="vertical" onFinish={handlePassword}>
          <Form.Item name="oldPassword" label={t('profile.oldPwd')} rules={[{ required: true }]}>
            <Input.Password prefix={<LockOutlined />} />
          </Form.Item>
          <Form.Item name="newPassword" label={t('profile.newPwd')} rules={[{ required: true, min: 6 }]}>
            <Input.Password prefix={<LockOutlined />} />
          </Form.Item>
          <Form.Item
            name="confirmPassword"
            label={t('profile.confirmPwd')}
            ...
            rules={[
              ...
              ({ getFieldValue }) => ({
                validator(_, value) {
                  ...
                  return Promise.reject(new Error(t('profile.pwdMismatch')));
                },
              }),
            ]}
          >
